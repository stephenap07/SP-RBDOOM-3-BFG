...
void idProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float launchPower, const float dmgPower )
{
	...
	if( !common->IsClient() || fl.skipReplication || this->clientsideNode.InList() )
	{
		if( fuse <= 0 )
		{
			// run physics for 1 second
			RunPhysics();
			if( common->IsClient() && this->clientsideNode.InList() )
			{
...
void idProjectile::AddDefaultDamageEffect( const trace_t& collision, const idVec3& velocity )
{
	...
	if( common->IsServer() && ( ( fl.networkSync && !gameLocal.mpGame.IsGametypeCoopBased() ) || ( fl.coopNetworkSync && gameLocal.mpGame.IsGametypeCoopBased() ) ) )
	{
		idBitMsg	msg;
...
void idProjectile::Explode( const trace_t& collision, idEntity* ignore )
{
	...
	if( *light_shader )
	{
		...
		if( common->IsClient() && this->clientsideNode.InList() )
		{
			...
			lightEndTime = MSEC_ALIGN_TO_FRAME( gameLocal.realClientTime + SEC2MS( light_fadetime ) );
		}
		else
		{
...
void idProjectile::ClientThink( const int curTime, const float fraction, const bool predict )
{
	...
	{
		if( !renderEntity.hModel && ( ( owner.GetEntity() && ( owner.GetEntity()->entityNumber != gameLocal.GetLocalClientNum() ) ) || !gameLocal.mpGame.IsGametypeCoopBased() ) )
		{
			return;
		}
		if( ( owner.GetEntity() && ( owner.GetEntity()->entityNumber != gameLocal.GetLocalClientNum() ) ) && gameLocal.mpGame.IsGametypeCoopBased() )
		{
			thinkFlags |= TH_THINK;
		}
...
================
*/
void idGuidedProjectile::WriteToSnapshot( idBitMsg& msg ) const
{
...
================
*/
void idSoulCubeMissile::WriteToSnapshot( idBitMsg& msg ) const
{
	idProjectile::WriteToSnapshot( msg );
}
...
================
*/
void idSoulCubeMissile::ReadFromSnapshot( const idBitMsg& msg )
{
	idProjectile::ReadFromSnapshot( msg );
}
